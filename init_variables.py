...
class Propagation(Pulse):
    ...
    def split_step(self):
        ...
        self.UP = np.float32(np.unwrap(np.angle(self.U)))
        # # Phase blanking
...
def plot_envelope2(x,y, pulsetype,colors,mode = 'time', z0 = 0, xrange = [-8, 8],  yrange = [0, 1.1], size_img = 600):
    if mode == 'time':
        x_title = 'T/T0'
        y_title = '|U(z,T)|^2'
        #updatemenus = []
...
def create_inital_base():
    pass
    #pd.DataFrame([vars(class) for class in pulse])
