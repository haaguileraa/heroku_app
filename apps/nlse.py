...
import sys
print('SIZE: ',sys.getsizeof(UI),sys.getsizeof(UIW), sys.getsizeof(UP), sys.getsizeof(Z))
print('Size: ', U.shape, UW.shape, UP.shape, Z.shape)
print('Type: ', U.dtype, UW.dtype, UP.dtype, Z.dtype)
...
prop_t = pulse.plot_propagation(mode = 'time') # propagation plot in time (amplitude)
prop_p = pulse.plot_propagation(mode = 'phase') # propagation plot in time (phase)
    ...
    children=[
        ...
        children=[
            ...
            children = [
                    ...
                    html.H3('Colormap Phase'),
                    dcc.Loading(id="loading3",children=[html.Div([dcc.Graph(id='propagationp', #id for callback purposes
                                                                            animate=True,
                                                                            figure=prop_p.update_layout()
                                                                            )   ])
                                                                ],type="circle",),                                     
...
def update_envelope(new_z,data,switch):
    ...
    Zi = data.get('zi', Z)
    UIn = data.get('u', UI)
    UIWn = data.get('uw', UIW)
    #Un = data.get('u', U)
    #UWn = data.get('uw', UW)
    #UIn =  np.float32(np.absolute(Un)**2)
    #UIWn =  np.float32(np.absolute(UWn)**2)
    return [plot_envelope2(T/T0,UIn, pulsetype,colors,mode = 'time', z0 = new_z),
            plot_envelope2(W,UIWn, pulsetype,colors,mode = 'spectrum', z0 = new_z),
